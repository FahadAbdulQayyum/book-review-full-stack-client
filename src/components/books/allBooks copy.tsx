                ...
                <div className='card bg-light'>
                    <h3 className='text-primary text-left'>
                        ...
                        {isAuthenticated ? <p className='side-by-side'>
                            ...
                            {reviews && reviews.map(rv => {
                                if (rv.bookId === v._id) {
                                    ...
                                    return (<><button className='btn btn-dark btn-sm btn-block'>Dislike</button>
                                        <button className='btn btn-danger btn-sm btn-block'>Dislike</button></>
                                    )
                                }
                            })}
                        </p> : <button className='btn btn-danger btn-sm btn-block' >Login to Vote</button>}
                        ...
                        {isAuthenticated ? <p className='side-by-side'>
                            ...
                            {/* <button className='btn btn-dark btn-sm btn-block' onClick={() => onLike(v._id)}>Like</button>
                            <button className='btn btn-danger btn-sm btn-block'>Dislike</button> */}
                            {/* {(user !== null && ((user.reviews !== undefined) && user.reviews.bookId === v._id) && user.reviews.like) ? <button className='btn btn-dark btn-sm btn-block' onClick={() => onLike(v._id)}>Like</button> */}
                            {user && user!.reviews!.bookId === v._id && user!.reviews!.like
                                ?
                                <button className='btn btn-danger btn-sm btn-block' onClick={() => onLike(v._id)}>Dislike</button>
                                : user && user!.reviews!.bookId === v._id && !user!.reviews!.like
                                    ?
                                    ...
                                    :
                                    (<><button className='btn btn-dark btn-sm btn-block'>Dislike</button>
                                        <button className='btn btn-danger btn-sm btn-block'>Dislike</button></>
                                    )}
                        </p> :
                            <p className='side-by-side'>
